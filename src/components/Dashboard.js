...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const newBalanceHistory = [...balanceHistory, { date: timestamp, balance: currentBalance }];
      
      console.log("Neue Balance History:", newBalanceHistory);  // Debug-Log hinzufügen
    ...
    const fetchEvents = async () => {
      try {
        ...
        const eventsWithCountdown = upcoming.map(event => {
          ...
          return { ...event, countdown: `${diffDays} Tage, ${diffHours} Stunden, ${diffMinutes} Minuten` };
        });
  ...
  const renderBalanceChart = () => {
    const dates = balanceHistory.map((entry) => new Date(entry.date).toLocaleDateString());
    ...
    <div className="dashboard">
        ...
        {visibleSections.transactions && (
          <div className="dashboard-section">
            <div className="section-header">
              ...
              <EyeOff className="section-toggle" onClick={() => toggleSection('transactions')} />
            </div>
              ...
              {recentEntries.map((entry, index) => {
                ...
                return (
                  <li key={index} className={`transaction-item ${entry.type}`}>
                    ...
                    <span>{new Date(entry.date).toLocaleString('de-DE', {
                      day: '2-digit',
                      month: '2-digit',
                      year: 'numeric',
                      hour: '2-digit',
        ...
        {visibleSections.events && (
          <div className="dashboard-section">
            <div className="section-header">
              ...
              <EyeOff className="section-toggle" onClick={() => toggleSection('events')} />
            </div>
      ...
      {visibleSections.charts && (
          <div className="dashboard-section ">
            <div className="section-header">
              <h3>Kategorien</h3>
              <EyeOff className="section-toggle" onClick={() => toggleSection('charts')} />
            </div>
