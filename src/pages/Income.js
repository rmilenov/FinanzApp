...
const Income = () => {
  ...
  const saveEntries = (data) => {
    const newList = editingEntry
      ...
      : [...entries, data];
    setEntries(newList);
    localStorage.setItem('income_entries', JSON.stringify(newList));
  };

  const handleDelete = (id) => {
    const filtered = entries.filter((e) => e.id !== id);
    setEntries(filtered);
    localStorage.setItem('income_entries', JSON.stringify(filtered));
  };

  return (
    <div>
      <h1>Einnahmen</h1>
      <EntryList entries={entries} onEdit={(e) => { setEditingEntry(e); setModalOpen(true); }} onDelete={handleDelete} type="income" />
